...
const AdminHero = ({
  ...
  return (
    ...
    >
      <div
        className={`relative mx-auto max-w-7xl ${alignClass} flex flex-col ${alignItems}`}
      >
        {title && (
        ...
        {subtitle && (
          <p className="text-lg md:text-xl mb-6 opacity-90 text-white max-w-3xl">
            {subtitle}
          </p>
        )}
          ...
          <div className={`flex flex-wrap ${alignFlex} gap-4 mb-8`}>
            {buttons.map((btn, idx) => (
              <a
                key={idx}
                href={btn.url || "#"}
                className={getButtonClass(btn.style || "primary")}
              >
                {btn.text}
              </a>
            ))}
