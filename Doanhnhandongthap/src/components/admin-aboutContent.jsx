...
const Info = ({ infoItems = [] }) => {
  const allInfo = useMemo(
    () => (Array.isArray(infoItems) ? infoItems : []),
    [infoItems],
  );
    ...
    <div className="flex flex-col h-auto p-4 gap-4">
      <div className="flex flex-col h-auto gap-4">
        {infoData.map((info, idx) => (
          <div
            key={idx}
            className="w-full h-auto bg-[#ffffff1f] backdrop-blur-[30px] shadow-[0_8px_32px_0_rgba(0,0,0,0.2)] flex items-center gap-4 p-4 overflow-hidden rounded-l-[100px] hover:-translate-y-0.75 hover:shadow-[0 8px 25px rgba(10, 37, 64, 0.08)] transition-all duration-300 ease-in-out"
          >
        ...
        {Array.isArray(container) &&
          container.map((item, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`w-full bg-[#ffffff] border border-blue-300 backdrop-blur-[30px] h-auto shadow-[0_8px_32px_0_rgba(0,0,0,0.2)] p-6 ${containerSideClasses}`}
                style={getContainerBackgroundStyle(item)}
              >
                <div
                  className={`${sizeH[title?.level] || sizeH[2]} font-bold mb-3`}
